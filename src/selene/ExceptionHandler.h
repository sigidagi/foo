...
inline void store_current_exception(lua_State * l, char const * what) {
    ...
    new(user_data) stored_exception{what};
...
public:
    using function = std::function<void(int,std::string)>;
...
public:
    ...
    void Handle(int luaStatusCode, std::string message) {
        if(_handler) {
            _handler(luaStatusCode, std::move(message));
        }
    }
